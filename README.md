...
- **教练工作台** `/coach`：今日 & 未来 7 天课程、本月完成课时
  - *课前简报*：会员健康档案与伤病限制、**本次约课填写的临时伤病限制**、套用模板、阶段体测差值、历史训练动作/负重/RPE（按上课时间倒序）、**上次遗留问题**（自动承接上节课的"下次重点"）、进行中目标
  - *课后登记*：约课时套用模板则**自动带入模板完整动作清单（组数/次数/负重，非统一 3×10）**，也可按部位从动作库预填；另含热身、RPE（1–10 中文描述）、承接遗留、课堂小结、下次重点；保存即完结；可标记爽约
...
- 角色权限：会员仅能操作本人数据，且时段列表不暴露其他预约会员姓名；教练只能管理本人课程（课前简报除外）；场地/课包仅管理员，时段/模板教练与管理员可维护
