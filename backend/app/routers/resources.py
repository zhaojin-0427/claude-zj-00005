...
from ..serializers import venue_dict, slot_dict, coach_dict, booking_dict
from .. import content
...
# ---------------- slots ----------------
def _visible_slot(s: Slot, db: Session) -> dict:
    booking = None
    if s.status == content.SLOT_BOOKED or s.status == content.SLOT_COMPLETED:
        b = db.scalar(select(Booking).where(Booking.slot_id == s.id))
        if b:
            booking = {"id": b.id, "member_id": b.member_id,
                       "member_name": db.get(User, b.member_id).full_name}
    return slot_dict(s, booking=booking)
...
def list_slots(
    ...
    return [_visible_slot(s, db) for s in slots]
...
def delete_slot(slot_id: int, db: Session = Depends(get_db),
    ...
    if s.status in (content.SLOT_BOOKED, content.SLOT_COMPLETED):
